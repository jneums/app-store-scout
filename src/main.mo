...
import MockAppStoreAdapter "MockAppStoreAdapter";
import LiveRegistryAdapter "LiveRegistryAdapter";
import McpRegistry "McpRegistry";

// Import tool modules
import ToolContext "tools/ToolContext";
import SearchApps "tools/search_apps";
import GetAppDetails "tools/get_app_details";
import GetCertificateSummary "tools/get_certificate_summary";
import ListRecentReleases "tools/list_recent_releases";
import ListCategories "tools/list_categories";
import ListTags "tools/list_tags";
import CompareApps "tools/compare_apps";
...
shared ({ caller = deployer }) persistent actor class McpServer(
  ...
  var registryCanisterIdText : Text = McpRegistry.mainnetCanisterIdText;
  ...
  var resourceContents = [
    ("file:///README.md", "# App Store Scout\n\nA Prometheus-native MCP server for app discovery, comparison, and certificate inspection."),
    ("file:///DATA_SOURCE.md", "Mock indexed dataset for MVP. Replace with official App Store adapters in Phase 2."),
  ];
  ...
  transient let resources : [McpTypes.Resource] = [
    {
      uri = "file:///README.md";
      name = "README.md";
      title = ?"App Store Scout Overview";
      description = ?"Overview and purpose of the App Store Scout MCP server.";
      mimeType = ?"text/markdown";
    },
    {
      uri = "file:///DATA_SOURCE.md";
      name = "DATA_SOURCE.md";
      title = ?"Data Source Notes";
      description = ?"Explains that MVP uses a mock indexed dataset pending live App Store adapters.";
      mimeType = ?"text/markdown";
    },
  ];
  ...
  transient let tools : [McpTypes.Tool] = [
    SearchApps.config(),
    GetAppDetails.config(),
    GetCertificateSummary.config(),
    ListRecentReleases.config(),
  ...
  transient let mcpConfig : McpTypes.McpConfig = {
    ...
    tools = tools;
    toolImplementations = [
      ("search_apps", SearchApps.handle(toolContext)),
      ("get_app_details", GetAppDetails.handle(toolContext)),
      ("get_certificate_summary", GetCertificateSummary.handle(toolContext)),
      ("list_recent_releases", ListRecentReleases.handle(toolContext)),
      ("list_categories", ListCategories.handle(toolContext)),
      ("list_tags", ListTags.handle(toolContext)),
      ("compare_apps", CompareApps.handle(toolContext)),
    ];
  ...
  public query func get_live_mode() : async Bool { return liveMode };

  public query func get_registry_canister_id() : async Principal {
    Principal.fromText(registryCanisterIdText)
  };
  ...
  public shared ({ caller }) func set_live_mode(new_mode : Bool) : async Result.Result<(), Payments.TreasuryError> {
    if (caller != owner) { return #err(#NotOwner) };
    liveMode := new_mode;
    return #ok(());
  };

  public shared ({ caller }) func set_registry_canister_id(new_id : Principal) : async Result.Result<(), Payments.TreasuryError> {
    if (caller != owner) { return #err(#NotOwner) };
    registryCanisterIdText := Principal.toText(new_id);
    return #ok(());
  };
